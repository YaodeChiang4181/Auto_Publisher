...
const vapidKeys = {
    publicKey: process.env.VAPID_PUBLIC_KEY || web_push_1.default.generateVAPIDKeys().publicKey,
    privateKey: process.env.VAPID_PRIVATE_KEY || web_push_1.default.generateVAPIDKeys().privateKey
};
web_push_1.default.setVapidDetails('mailto:admin@autopublisher.local', vapidKeys.publicKey, vapidKeys.privateKey);
...
server.get('/api/qr/token', async (request, reply) => {
    ...
    await redis_1.redis.setex(`qr_token:${token}`, 10, tokenData);
...
server.get('/api/qr/scan', async (request, reply) => {
    const { token, geoLat, geoLng } = request.query;
    if (!token) {
        return reply.status(400).send({ error: 'Token is required' });
    }
    // Verify the token in Redis
    const tokenDataStr = await redis_1.redis.get(`qr_token:${token}`);
    if (!tokenDataStr) {
    ...
    const browserToken = crypto_1.default.randomBytes(32).toString('hex');
    ...
    await redis_1.redis.setex(`session_timelock:${browserToken}`, 300, 'locked'); // 300 seconds as example
    // After successful scan, we might also want to invalidate the QR token immediately (Single-use)
    await redis_1.redis.del(`qr_token:${token}`);
    return {
        success: true,
...
server.post('/api/push/subscribe', async (request, reply) => {
    const { browserToken, subscription } = request.body;
...
server.get('/api/session/status', async (request, reply) => {
    const { browserToken } = request.query;
    if (!browserToken)
        return reply.status(400).send({ error: 'Missing browserToken' });
    const session = await prisma_1.prisma.session.findUnique({ where: { browserToken } });
    if (!session)
...
const start = async () => {
    try {
        ...
        // 啟用 Redis 的過期事件通知 (Ex)
        await redis_1.redis.config('SET', 'notify-keyspace-events', 'Ex');
        // 訂閱過期事件
        redisSub.subscribe('__keyevent@0__:expired', (err) => {
            if (err)
                server.log.error('Failed to subscribe to Redis expired events', err);
        });
        // 處理「離場時間鎖」到期事件
        redisSub.on('message', async (channel, message) => {
            if (message.startsWith('session_timelock:')) {
                ...
                try {
                    ...
                    if (session.pushSub) {
                        await web_push_1.default.sendNotification(session.pushSub, JSON.stringify({
                            title: '彩蛋已解鎖！',
